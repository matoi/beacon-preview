...
(ert-deftest beacon-preview-dwim-jumps-to-block-when-preview-is-live-and-current ()
  (with-temp-buffer
    ...
    (let ((jumped nil)
      ...
      (cl-letf (((symbol-function 'beacon-preview-build-and-open)
                ...
                ((symbol-function 'beacon-preview--tracked-preview-window)
...
(ert-deftest beacon-preview-dwim-reclaims-hidden-preview-window-without-building-when-current ()
  (with-temp-buffer
    (setq-local major-mode 'markdown-mode)
      ...
      (unwind-protect
          (progn
            ...
            (cl-letf (((symbol-function 'beacon-preview-build-and-open)
              ...
              (beacon-preview-dwim)
              (should-not built)
              (should (eq shown preview-buffer))
...
(ert-deftest beacon-preview-dwim-reclaims-hidden-preview-window-and-refreshes-html-when-stale ()
  (with-temp-buffer
    (setq-local major-mode 'markdown-mode)
    (setq-local buffer-file-name "/tmp/sample.md")
    (let ((source-buffer (current-buffer))
      ...
      (unwind-protect
          (progn
            ...
            (cl-letf (((symbol-function 'beacon-preview-build-current-file)
                       (lambda () '(:html "/tmp/updated.html")))
                      ((symbol-function 'beacon-preview--xwidget-available-p)
                       (lambda () t))
                      ...
                      ((symbol-function 'xwidget-webkit-browse-url)
                       (lambda (&rest _args)
                         (ert-fail "dwim should reuse the existing preview session")))
              ...
              (beacon-preview-dwim)
              (should (eq (window-buffer preview-window) preview-buffer))
              (should (string-match-p "updated\\.html$" goto-url))))
        (when (window-live-p preview-window)
          (delete-window preview-window))
        (when (buffer-live-p preview-buffer)
          (kill-buffer preview-buffer))
